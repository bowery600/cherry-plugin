...
export default function Edit( { attributes, setAttributes } ) {
	const { eyebrow, heading, lede, committeeFilter, maxItems } = attributes;
		...
		<>
			<InspectorControls>
				...
				>
					...
					<TextControl
						label={ __(
							'Filter by committee (optional)',
							'cherrystone-blocks'
						) }
						help={ __(
							'Exact committee name, e.g. Healthcare & Life Sciences. Leave blank for all.',
							'cherrystone-blocks'
						) }
						value={ committeeFilter }
						onChange={ ( value ) =>
							setAttributes( { committeeFilter: value } )
						}
					/>
					<p>
