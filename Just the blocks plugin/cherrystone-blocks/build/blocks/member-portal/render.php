				...
				<div class="container">
					<div class="member-portal-tabs">
						<div class="member-portal-tab-list" aria-label="<?php esc_attr_e( 'Member portal sections', 'cherrystone-blocks' ); ?>">
							<a class="member-portal-tab is-active" href="<?php echo esc_url( home_url( '/member-portal' ) ); ?>" aria-current="page"><?php esc_html_e( 'Info & Resources', 'cherrystone-blocks' ); ?></a>
							<a class="member-portal-tab" href="<?php echo esc_url( home_url( '/member-directory' ) ); ?>"><?php esc_html_e( 'Member Directory', 'cherrystone-blocks' ); ?></a>
						</div>
						<a class="member-portal-signout mono" href="<?php echo esc_url( add_query_arg( 'cs_member_logout', '1', home_url( '/member-portal' ) ) ); ?>"><?php esc_html_e( 'Sign out ->', 'cherrystone-blocks' ); ?></a>
					</div>
...
<section <?php echo $wrapper; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped by get_block_wrapper_attributes(). ?> style="padding-bottom:140px;">
	<div class="container" style="max-width:560px;">
		...
		<form method="post" action="<?php echo esc_url( $form_action ); ?>" style="margin-top:36px;padding:32px;border:1px solid var(--line);border-radius:var(--radius-lg);background:var(--surface);display:flex;flex-direction:column;gap:16px;">
			...
			>
			<?php if ( $failed ) : ?>
				<div style="font-size:13px;color:var(--accent);font-family:var(--font-mono);">
