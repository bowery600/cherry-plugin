...
require_once CHERRYSTONE_BLOCKS_PATH . 'includes/acf-fields.php';
...
function cherrystone_blocks_register_content_types() {
	...
	register_post_type(
		...
		array(
			'labels'       => array(
				'name'          => __( 'Members', 'cherrystone-blocks' ),
				'singular_name' => __( 'Member', 'cherrystone-blocks' ),
				'add_new_item'  => __( 'Add Member', 'cherrystone-blocks' ),
				'edit_item'     => __( 'Edit Member', 'cherrystone-blocks' ),
			),
			'description'  => __( 'Cherrystone members, angels, and advisors. Members flagged "Is Leadership" also appear on the Leadership page.', 'cherrystone-blocks' ),
...
/**
 * One-time migration: merge cherry_leader posts into the unified cherry_member type.
 *
 * Leadership and regular members now share the cherry_member post type and the
 * cs_member_* meta keys; leadership is flagged with cs_is_leadership = '1'. For each
 * legacy cherry_leader post this copies its richer role/LinkedIn/bio meta onto the
 * matching cherry_member post (matched by title) and deletes the leader post, or —
 * when no member post exists — converts the leader post in place.
 */
function cherrystone_blocks_migrate_unified_members() {
	if ( get_option( 'cherrystone_member_unification_v2' ) ) {
		return;
	}

	global $wpdb;

	// cherry_leader is no longer registered, so query the table directly.
	$leaders = $wpdb->get_results(
		"SELECT ID, post_title, menu_order FROM {$wpdb->posts} WHERE post_type = 'cherry_leader' AND post_status NOT IN ( 'trash', 'auto-draft' )"
	);

	// Leader titles that differ from the matching member post title.
	$title_aliases = array(
		'Steve Blough' => 'Stephen Blough',
	);

	$meta_map = array(
		'cs_leader_role'         => 'cs_member_role',
		'cs_leader_linkedin_url' => 'cs_member_linkedin_url',
		'cs_leader_photo_url'    => 'cs_member_photo_url',
		'cs_leader_description'  => 'cs_member_description',
	);

	foreach ( $leaders as $leader ) {
		$member_title = isset( $title_aliases[ $leader->post_title ] ) ? $title_aliases[ $leader->post_title ] : $leader->post_title;

		$member_id = (int) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT ID FROM {$wpdb->posts} WHERE post_type = 'cherry_member' AND post_status = 'publish' AND post_title = %s LIMIT 1",
				$member_title
			)
		);

		if ( $member_id ) {
			// Merge the richer leader meta onto the existing member post.
			foreach ( $meta_map as $leader_key => $member_key ) {
				$value = get_post_meta( $leader->ID, $leader_key, true );
				if ( '' === $value ) {
					continue;
				}

				if ( 'cs_member_description' === $member_key ) {
					// Keep the member card's "Expertise: …" line after the full bio.
					$existing = get_post_meta( $member_id, 'cs_member_description', true );
					if ( $existing && 0 === strpos( trim( $existing ), 'Expertise:' ) && false === strpos( $value, trim( $existing ) ) ) {
						$value .= "\n\n" . trim( $existing );
					}
				}

				update_post_meta( $member_id, $member_key, $value );
			}

			update_post_meta( $member_id, 'cs_is_leadership', '1' );

			// Carry over the leadership display order (the directory sorts by title).
			wp_update_post(
				array(
					'ID'         => $member_id,
					'menu_order' => (int) $leader->menu_order,
				)
			);

			wp_delete_post( $leader->ID, true );
			clean_post_cache( $member_id );
			continue;
		}

		// No matching member: convert the leader post in place.
		$wpdb->update(
			$wpdb->posts,
			array( 'post_type' => 'cherry_member' ),
			array( 'ID' => $leader->ID ),
			array( '%s' ),
			array( '%d' )
		);
		...
		foreach ( $meta_map as $leader_key => $member_key ) {
			$value = get_post_meta( $leader->ID, $leader_key, true );
			if ( '' !== $value ) {
				update_post_meta( $leader->ID, $member_key, $value );
			}
			delete_post_meta( $leader->ID, $leader_key );
		}

		update_post_meta( $leader->ID, 'cs_is_leadership', '1' );
		clean_post_cache( $leader->ID );
	}

	update_option( 'cherrystone_member_unification_v2', '1' );
	flush_rewrite_rules();
}
add_action( 'admin_init', 'cherrystone_blocks_migrate_unified_members', 5 );
...
function cherrystone_blocks_sanitize_meta_value( $value, $key ) {
	if ( in_array( $key, array( 'cs_logo_url', 'cs_company_url', 'cs_registration_url', 'cs_resource_url', 'cs_linkedin_url', 'cs_sponsor_url', 'cs_testimonial_logo_url', 'cs_people_photo_url', 'cs_sponsor_logo_url', 'cs_testimonial_photo_url', 'cs_member_photo_url', 'cs_member_linkedin_url' ), true ) ) {
		return esc_url_raw( $value );
	}

	if ( in_array( $key, array( 'cs_agenda', 'cs_company_description', 'cs_people_description', 'cs_resource_description', 'cs_testimonial_quote', 'cs_member_description' ), true ) ) {
		return sanitize_textarea_field( $value );
	}

	if ( in_array( $key, array( 'cs_member_only', 'cs_is_chair', 'cs_is_leadership' ), true ) ) {
		return $value ? '1' : '';
	}
...
function cherrystone_blocks_admin_media_scripts( $hook ) {
	...
	if ( in_array( $pt, array( 'cherry_portfolio', 'cherry_member', 'cherry_sponsor', 'cherry_pitch_event', 'cherry_resource', 'cherry_communication', 'cherry_testimonial' ), true ) ) {
		wp_enqueue_media();
	}
}
...
/**
 * Render details for Members on single page view.
 */
		...
		<div class="cs-tab-content" id="tab-dashboard">
			...
			<div class="cs-dashboard-summary">
				...
				<div class="cs-stat-card">
					...
					<div class="cs-stat-desc"><?php esc_html_e( 'Members flagged "Is Leadership".', 'cherrystone-blocks' ); ?></div>
				...
				<div class="cs-stat-card cherry">
					...
					<div class="cs-stat-label"><?php esc_html_e( 'Members', 'cherrystone-blocks' ); ?></div>
