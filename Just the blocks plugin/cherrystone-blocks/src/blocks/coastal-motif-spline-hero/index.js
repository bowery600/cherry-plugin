...
const deprecated = [
	{
		attributes: {
			...
			headline: {
				type: 'string',
				default: 'Early-stage capital for New England founders.',
			},
			subheading: {
				type: 'string',
				default:
					'We bring pooled angel funding, sector expertise, and board leadership to accelerate regional startups from screening to scale.',
			},
			...
			btnSecondaryLink: { type: 'string', default: '/members' },
		...
		migrate( attributes ) {
			return {
				...attributes,
				videoSrc: '/wp-content/uploads/output.webm',
			};
		},
		save( { attributes } ) {
			const {
			...
			return (
				<>
					...
					<section { ...blockProps }>
						...
						<div
							className="sequence-scroll-indicator"
							aria-hidden="true"
						>
